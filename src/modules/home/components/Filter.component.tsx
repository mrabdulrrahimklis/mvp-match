...
import { CustomSelect, CustomButton, CustomMenuItem, CustomFormControl, CustomInputLabel, CustomTextField } from "../../../core/styled";
...
import {IReportParams, requestGetReports} from "../store/saga/request/report.request";
...
export const FilterComponent = () => {
    ...
    const getData = async () => {
        ...
        const { data }: AxiosResponse<IResponse<IReport>> = await requestGetReports(reportParams);
        ...
        <Box display='flex'>
            <Box width='135px' marginLeft='23px'>
                <CustomFormControl fullWidth size='small'>
                    ...
                    >
                        <CustomMenuItem value={JSON.stringify({
                            projectId: '',
                            projectName: 'All projects'
                        })}>All projects</CustomMenuItem>
                        {projects?.map((project: IProject, index) => (<CustomMenuItem key={index} value={JSON.stringify({
                            projectId: project.projectId,
                            projectName: project.name
                        })}>{project.name}</CustomMenuItem>))}
                    </CustomSelect>
                </CustomFormControl>
            </Box>
            <Box width='135px' marginLeft='23px'>
                <CustomFormControl fullWidth size='small'>
                    ...
                    >
                        ...
                        })}>All gateways</CustomMenuItem>
                        {gateways?.map((gateway: IGateway, index) => (<CustomMenuItem key={index} value={JSON.stringify({
                            gatewayId: gateway.gatewayId,
                            gatewayName: gateway.name
                        })}>{gateway.name}</CustomMenuItem>))}
                    </CustomSelect>
                </CustomFormControl>
            </Box>
            ...
            <Box marginLeft='23px'>
                <CustomButton onClick={() => getData()} variant='contained' >Generate Report</CustomButton>
            </Box>
        </Box>
    </Box>
}